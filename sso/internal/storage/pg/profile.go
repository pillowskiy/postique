...
func (s *ProfileStorage) ProfileByUsername(ctx context.Context, username string) (*domain.UserProfile, error) {
	const op = "ph.ProfileStorage.ProfileByUsername"
	q, args, err := psql.Select("*").From("profiles").Where("username = ?", username).ToSql()
	if err != nil {
		return nil, fmt.Errorf("%s: %w", op, err)
	}
